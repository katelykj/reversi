...
/* What to do when the server someone joined a room */
...
socket.on('join_room_response', function (payload){
  ...
  /* Manage a message that a new player has joined */
...
/* What to do when a server has left the room */
...
socket.on('player_disconnected', function (payload){
  ...
  /* If we are being notified that we have left the room, then ignore it */
  if(payload.socket_id == socket.id){
    return;
  }
  ...
  if(dom_elements.length != 0){
    ...
    /* Manage a message if player has left */
...
function send_message(){
  ...
  $(function(){
    ...
    $('#quit').append('<a href ="lobby.html?username='+username+'" class="btn btn-danger btn-defult active" role="button" aria-pressed="true">Quit</a>');

  });


  /* Code for the board specify */
      ...
      /* If a board has changed*/
        ...
        if(old_board[row][column] !=board [row][column]){
            if(old_board[row][column] == '?' && board [row][column] == ' '){
               ...
               $('#'+row+'_'+column).html('<img src="assets/images/pink_to_blue.gif" alt="blue square"/>');
            }
            else if(old_board[row][column] == 'b' && board [row][column] == 'p'){
               $('#'+row+'_'+column).html('<img src="assets/images/blue_to_pink.gif" alt="pink square"/>');
            }
            else{
...
$('#'+row+'_'+column).removeClass('hover_over');

if(payload.game.whose_turn === my_color){
  if(payload.game.legal_moves[row][column] === my_color.substr(0,1)){
                $('#'+row+'_'+column).addClass('hover_over');
  ...
  socket.on('play_token_response', function(payload){
    ...
    /* check for a good play token resopnse*/
